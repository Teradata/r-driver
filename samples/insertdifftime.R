...
# This sample program demonstrates how to insert R difftime values into a temporary table using the DBI::dbWriteTable method.
...
main <- function () {
	...
	tryCatch ({
		...
		# the destination column data type defaults to VARCHAR and can be overriden with the field.types argument
		DBI::dbWriteTable (con, sTableName, df, field.types = "interval day to second", temporary = TRUE)
