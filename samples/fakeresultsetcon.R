...
main <- function () {
	...
	con <- DBI::dbConnect (teradatasql::TeradataDriver (), host = "whomooz", user = "guest", password = "please", fake_result_sets = TRUE)
	...
	tryCatch ({
		...
		res <- DBI::dbSendQuery (con, "select * from voltab1 order by 1")
		...
		res <- DBI::dbSendQuery (con, "select * from voltab2 order by 1 ; select * from voltab3 order by 1")
