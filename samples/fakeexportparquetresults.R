...
main <- function () {
	...
	tryCatch ({
		...
		sRequest <- paste0 ("{fn teradata_write_parquet(", parquetFileName, ")}{fn teradata_fake_result_sets}select * from voltab where c1 < 5 order by 1;select * from voltab where c1 >= 5 order by 1;select 'abc' as col1, '12' as col2")
		cat (paste0 (sRequest, "\n"))
		DBI::dbExecute (con, sRequest)

		tryCatch ({

			# When using fake_result_sets escape function, two Parquet files are produced for each statement
			listFileNames <- list ("dataR.parquet", "dataR_1.parquet", "dataR_2.parquet", "dataR_3.parquet", "dataR_4.parquet", "dataR_5.parquet")
